...
final class APIClient {
  ...
  init() {
    ...
    //chat.startChat(withAccountKey: "KyUHKFutTJdfOlCQBpgrIvMgke7uuehK", config: config)
    //chat.startChat(withAccountKey: "ll5k619QxF8fjp3lpMZ7n8nlI6Hych3e", config: config)
    //chat.startChat(withAccountKey: "vdmaAclor4ZF3RXYyoi2gxIaYP95F1k1", config: config)
    //chat.startChat(withAccountKey: "VsPFE0szOqswtjf2Uc8NEuhcQMhULLjE", config: config)
    
    //Paid Account Key Zendesk
    chat.startChat(withAccountKey: "MpzFQA7e6CzxRsL6OzuOfxGwCcSVmvIV", config: config)
  ...
  fileprivate func handleChatEvent(_ event: ZDCChatEvent) {
    print("Received chat event \(event.eventId ?? "NO_ID") of type \(event.type.rawValue) with: \(event.message ?? "NO_MSG") time stampcvalue : \(String(describing: event.timestamp))")
  ...
  func sendMessage(_ text: String) {
    print("sendMessage api client : ", self.chat.connectionStatus, text)
